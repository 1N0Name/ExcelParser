...
bool SheetColumnsModel::setColumnAction(int index, ActionType action, const QString& customName) {
    ...
    if (index < 0 || index >= m_columns.size() || (action == KeySource && isKeySourceAlreadySet())) {
        LOGD("Invalid index or KeySource already set. Index: " << index << ", Action: " << action);
        return false;
    }
    ...
    LOGD("Action set for Index: " << index << " Action: " << action << " CustomName: " << customName);
    return true;
}

bool SheetColumnsModel::isKeySourceAlreadySet() const {
    for (const auto& column : m_columns) {
        if (column.actionType == KeySource) return true;
    }
    return false;
}
